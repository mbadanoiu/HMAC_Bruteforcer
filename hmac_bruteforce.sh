...
if [[ -z "$1" || -z "$2" || -z "$3" || -z "$4" ]]; then
	echo -e "\n\tUsage: $0 <Wordlist> <HMAC> <md4/md5/sha1/sha256/sha512> <Message_File>\n"
	exit
fi
...
if [[ ! -s "$1" ]]; then
	echo -e "\n\tFile $1 doesn't exist or is empty\n"
	exit
fi
...
while read i; do
	multithread "$i"
done <<< "$(cat $wordlist; echo Done reading $wordlist! Running hmac-$mode 1>&2)"
